...
func build() -> void:
	var mesh := grid_mesh(-130.0, 70.0, -WorldContext.CHUNK * 2.2, WorldContext.CHUNK * 1.2, 2.0, ctx.sand_height)
	...
	for i in count:
		...
		var t := Transform3D(Basis(Vector3.UP, rng.randf() * TAU).scaled(Vector3(rng.randf_range(0.8, 2.2), 0.6, rng.randf_range(0.8, 1.6))), Vector3(x, ctx.sand_height(x, z) + 0.01, z))
