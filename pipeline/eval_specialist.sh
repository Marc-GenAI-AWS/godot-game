...
./wait_gpu.sh 60
PYTHONPATH=. .venv-train/bin/python loop/specialist.py --segment sky --revise runs/$RUN/verified.jsonl --out runs/$RUN --backend "hf:$DIR"
DISPLAY=:0 .venv/bin/python verify.py --candidates runs/$RUN/revisions.jsonl --out runs/$RUN --name verified_rev.jsonl --workers 3
.venv/bin/python - "$RUN" "$BRIEFS" <<'EOF2'
...
teacher = [json.loads(l) for f in glob.glob("runs/sky[0-9]/verified.jsonl") for l in open(f) if json.loads(l)["brief_id"] in ids]
