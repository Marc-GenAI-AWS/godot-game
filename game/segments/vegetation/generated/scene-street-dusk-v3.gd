...
func build_chunk(chunk: Node3D, rng: RandomNumberGenerator) -> void:
	...
	for side: float in [-1.0, 1.0]:
		...
		for i in count:
			...
			# Younger trees: scale down the whole leafy tree model so the
			# row reads as a bunch of small, thin plants rather than full
			# mature trees. A tiny bit of jitter keeps it from looking
			# like a grid of clones.
			var sz := rng.randf_range(0.45, 0.65)
			LeafyTree.build(chunk, Vector3(x, 0.0, z), rng, ctx, sz)
