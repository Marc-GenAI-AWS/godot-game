...
func build_chunk(chunk: Node3D, rng: RandomNumberGenerator) -> void:
	...
	for side: float in [-1.0, 1.0]:
		...
		var spacing := L / float(count)
		var z := -L + rng.randf_range(0.0, spacing)
		for i in count:
			var x := side * (sc.WALK_OUT + 1.6 + rng.randf_range(-0.4, 0.8))
			LeafyTree.build(chunk, Vector3(x, 0.0, z), rng, ctx, rng.randf_range(1.0, 1.3))
			z += spacing * rng.randf_range(0.7, 1.3)
			if z >= 0.0:
				break

		# some hedges along lot fronts, lush and irregular
		var hz := -L
		...
		while hz < 0.0:
			var len := rng.randf_range(4.0, 10.0)
			if rng.randf() < 0.4:
				var hh := rng.randf_range(0.9, 1.5)
				hb.add_box_at(Vector3(0.8, hh, len), Color(0.13, 0.32, 0.13), Vector3(side * (sc.LOT_X - 0.6), hh * 0.5, hz + len * 0.5))
				var n := int(len * 3.5)
				for j in n:
					var p := Vector3(side * (sc.LOT_X - 0.6) + rng.randf_range(-0.45, 0.45), rng.randf_range(0.2, hh + 0.25), hz + rng.randf_range(0.0, len))
				...
				ctx.add_obstacle(chunk.global_transform * Vector3(side * (sc.LOT_X - 0.6), 0.0, hz + len * 0.5), 0.8)
			hz += len + rng.randf_range(3.0, 8.0)
		if not hb.is_empty():
