...
func build_chunk(chunk: Node3D, rng: RandomNumberGenerator) -> void:
	...
	for side: float in [-1.0, 1.0]:
		...
		# some hedges along lot fronts, tidy municipal style
		...
		while hz < 0.0:
			if rng.randf() < 0.35:
				...
				var hh := rng.randf_range(0.7, 1.0)
				var hx := side * (sc.LOT_X - 0.6)
				hb.add_box_at(Vector3(0.75, hh, len), Color(0.13, 0.32, 0.14), Vector3(hx, hh * 0.5, hz + len * 0.5))
				var n := int(len * 2.5)
				for j in n:
					...
					shrubs.add(card, Transform3D(b, p), Color(1, 1, 1).lerp(Color(0.75, 0.92, 0.7), rng.randf()) * rng.randf_range(0.8, 1.05))
