...
# Sky dome, sun, fog/tonemapping and drifting cloud streaks for a dusk
# beach scene: flattened amber horizon fading into a magenta-violet zenith
# band, punctuated by thin streaky clouds with pink undersides.
...
func build() -> void:
	ctx.sky_zenith = Color(0.09, 0.10, 0.52)
	ctx.sky_horizon = Color(0.98, 0.42, 0.28)
	ctx.sun_color = Color(1.0, 0.52, 0.28)
	ctx.fog_color = Color(0.82, 0.5, 0.42)
	...
	sky_mat.ground_bottom_color = Color(0.10, 0.08, 0.14)
	sky_mat.ground_horizon_color = Color(0.72, 0.42, 0.32)
	...
	var azimuth_yaw: float = 180.0 - 90.0
	sun.rotation_degrees = Vector3(-1.5, azimuth_yaw, 0)
	add_child(sun)
	if sun.is_inside_tree():
		ctx.sun_dir = -sun.global_transform.basis.z
	else:
		ctx.sun_dir = Vector3(-0.95, -0.2, 0.0).normalized()

	var rng := RandomNumberGenerator.new()
	rng.seed = 91
	var cloud_count: int = int(round(0.25 * 40.0))
	for i in cloud_count:
		...
		s.shaded = false
		s.transparent = true
		s.pixel_size = rng.randf_range(1.2, 2.4)
		...
		s.modulate = Color(1.0, 0.72, 0.55, rng.randf_range(0.85, 1.0))
