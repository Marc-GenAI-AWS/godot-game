...
# Sky dome, sun, fog/tonemapping and drifting cloud sprites for a clear,
# bright tropical noon at mid-elevation, sun almost overhead but slightly
# to the right over the sea.
...
func build() -> void:
	ctx.sky_zenith = Color(0.10, 0.28, 0.62)
	ctx.sky_horizon = Color(0.62, 0.86, 0.95)
	ctx.sun_color = Color(1.0, 0.98, 0.92)
	ctx.fog_color = Color(0.68, 0.86, 0.93)
	...
	sky_mat.ground_horizon_color = Color(0.68, 0.86, 0.95)
	...
	for i in cloud_count:
		...
		s.shaded = false
		s.transparent = true
		s.pixel_size = rng.randf_range(0.7, 1.1)
		var ang := rng.randf_range(-PI, PI)
		var dist := rng.randf_range(550.0, 900.0)
		s.position = Vector3(cos(ang) * dist, rng.randf_range(80.0, 180.0), sin(ang) * dist)
