...
func build_chunk(chunk: Node3D, rng: RandomNumberGenerator) -> void:
	...
	for i in 8:
		...
		_add_baked(p, clip, _pick(idle_ts, rng), Transform3D(Basis(Vector3.UP, yaw), Vector3(wx, ctx.ground_height(wx, wz), wz)))
	...
	for i in 6:
		...
		for k in (2 if pair else 1):
			...
			var xx := x + k * 0.8
			root.position = Vector3(xx, ctx.ground_height(xx, z), z)
...
func tick(delta: float) -> void:
	for w in walkers:
		...
		root.position.x = wx
		root.position.z = z
		root.position.y = ctx.ground_height(wx, z)
