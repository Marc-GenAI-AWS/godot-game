...
func build() -> void:
	...
	asphalt_mat.set_shader_parameter("wear", 0.3)
	...
	for y in 256:
		for x in 256:
			...
			var stripe: float = 0.94 + 0.06 * signf(sin(x * 0.2))
			var c: Color = Color(0.07, 0.16, 0.09).lerp(Color(0.14, 0.24, 0.12), v) * stripe
			img.set_pixel(x, y, c)
...
func build_chunk(chunk: Node3D, rng: RandomNumberGenerator) -> void:
	...
	# paint: single dashed white centre line only, fresh and crisp
	var paint := MeshBatch.new()
	var white := Color(0.82, 0.82, 0.82)
	var z := -L
	while z < 0.0:
		var seg := rng.randf_range(6.0, 14.0)
		var dash_len := rng.randf_range(0.4, 0.7)
		var gap_len := rng.randf_range(0.4, 0.7)
		paint.add_box_at(Vector3(0.12, 0.012, seg), white * 0.95, Vector3(-0.15, 0.025, z + seg * 0.5))
		paint.add_box_at(Vector3(0.12, 0.012, seg), white * rng.randf_range(0.75, 1.0), Vector3(0.15, 0.025, z + seg * 0.5))
		paint.add_box_at(Vector3(dash_len, 0.012, seg), white * 0.95, Vector3(0.0, 0.025, z + seg * 0.5 + dash_len))
		paint.add_box_at(Vector3(gap_len, 0.012, seg), Color(0.16, 0.16, 0.165), Vector3(0.0, 0.025, z + seg * 0.5 + dash_len + gap_len))
		z += seg
	var cz := -L + 12.0
	for i in 10:
	...
	# kerbs (granite grey) and sidewalks in concrete, with long clean slabs
	...
	for side: float in [-1.0, 1.0]:
		...
		while zz < 0.0:
			var seg := 4.0
			kerb.add_box_at(Vector3(0.3, sc.KERB_H, seg - 0.03), Color(0.5, 0.48, 0.46), Vector3(kx, sc.KERB_H * 0.5, zz + seg * 0.5))  # Distinctly granite grey
			zz += seg
		var wx := side * (sc.ROAD_HALF + 0.3 + (sc.WALK_OUT - sc.ROAD_HALF - 0.3) * 0.5)
		var ww := sc.WALK_OUT - sc.ROAD_HALF - 0.3
		var z2 := -L
		while z2 < 0.0:
			var slab := 2.7  # Lengthened sidewalk slabs
			walk.add_box_at(Vector3(ww, sc.KERB_H, slab - 0.04), Color(0.5, 0.5, 0.48), Vector3(wx, sc.KERB_H * 0.5, z2 + slab * 0.5))  # Neutral grey slab albedo
			if rng.randf() < 0.08:
