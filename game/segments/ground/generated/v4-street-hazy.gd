...
# Brownish sun-baked asphalt with double yellow centre line and white edge
# lines, plain concrete kerbs, short cracked sidewalk slabs, dark mown
...
func build() -> void:
	...
	asphalt_mat.set_shader_parameter("base_color", Color(0.36, 0.33, 0.28))
	...
	# lawn: dark mown greens with mowing stripes
	...
	for y in 256:
		for x in 256:
			...
			var c: Color = Color(0.09, 0.2, 0.08).lerp(Color(0.16, 0.28, 0.12), v) * stripe
			img.set_pixel(x, y, c)
...
func build_chunk(chunk: Node3D, rng: RandomNumberGenerator) -> void:
	...
	# paint: double yellow centre line, white edge lines, faded and worn
	...
	while z < 0.0:
		...
		var fade := rng.randf_range(0.7, 1.0)
		paint.add_box_at(Vector3(0.12, 0.012, seg), yellow * fade, Vector3(-0.15, 0.025, z + seg * 0.5))
		paint.add_box_at(Vector3(0.12, 0.012, seg), yellow * rng.randf_range(0.7, 1.0), Vector3(0.15, 0.025, z + seg * 0.5))
		for side: float in [-1.0, 1.0]:
			paint.add_box_at(Vector3(0.12, 0.012, seg), white * rng.randf_range(0.7, 1.0), Vector3(side * (sc.ROAD_HALF - 2.3), 0.025, z + seg * 0.5))
		z += seg
	var cz := -L + 12.0
	for i in 10:
	...
	# plain concrete kerbs and short cracked sidewalk slabs
	...
	for side: float in [-1.0, 1.0]:
		...
		while z2 < 0.0:
			var slab := 1.6
			walk.add_box_at(Vector3(ww, sc.KERB_H, slab - 0.04), Color(1, 1, 1) * rng.randf_range(0.9, 1.0), Vector3(wx, sc.KERB_H * 0.5, z2 + slab * 0.5))
			if rng.randf() < 0.22:
				walk.add_box_at(Vector3(0.03, 0.006, rng.randf_range(0.5, 1.4)), Color(0.5, 0.5, 0.48), Vector3(wx + rng.randf_range(-ww * 0.4, ww * 0.4), sc.KERB_H + 0.003, z2 + rng.randf_range(0.2, 1.4)))
			z2 += slab
