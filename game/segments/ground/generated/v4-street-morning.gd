...
# Fresh black asphalt with a single dashed white centre line, plain concrete
# kerbs, long clean sidewalk slabs, and lush green lawns.
...
func build() -> void:
	...
	asphalt_mat.set_shader_parameter("stain_strength", 0.2)
	asphalt_mat.set_shader_parameter("patch_strength", 0.1)
	asphalt_mat.set_shader_parameter("wear", 0.3)

	# lawn: lush, saturated greens with mow stripes
	...
	for y in 256:
		for x in 256:
			...
			var c: Color = Color(0.16, 0.42, 0.14).lerp(Color(0.32, 0.58, 0.2), v) * stripe
			img.set_pixel(x, y, c)
...
func build_chunk(chunk: Node3D, rng: RandomNumberGenerator) -> void:
	...
	# paint: single dashed white centre line, no edge lines
	var paint := MeshBatch.new()
	var white := Color(0.85, 0.85, 0.83)
	var z := -L
	while z < 0.0:
		var dash := rng.randf_range(3.0, 5.0)
		var gap := rng.randf_range(2.0, 3.0)
		var fade := rng.randf_range(0.85, 1.0)
	...
	# plain concrete kerbs and long clean sidewalk slabs
	...
	for side: float in [-1.0, 1.0]:
		...
		while z2 < 0.0:
			var slab := 4.5
			walk.add_box_at(Vector3(ww, sc.KERB_H, slab - 0.04), Color(1, 1, 1) * rng.randf_range(0.96, 1.0), Vector3(wx, sc.KERB_H * 0.5, z2 + slab * 0.5))
			if rng.randf() < 0.02:
				walk.add_box_at(Vector3(0.03, 0.006, rng.randf_range(0.5, 1.4)), Color(0.5, 0.5, 0.48), Vector3(wx + rng.randf_range(-ww * 0.4, ww * 0.4), sc.KERB_H + 0.003, z2 + rng.randf_range(0.3, slab - 0.3)))
			z2 += slab
