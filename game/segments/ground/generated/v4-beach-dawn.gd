...
# Pale white coral sand beach: fine grain, wide wet band, sparse shell scatter.
...
func build() -> void:
	...
	material.set_shader_parameter("dry_color", Color(0.86, 0.8, 0.7))
	material.set_shader_parameter("wet_color", Color(0.32, 0.27, 0.2))
	material.set_shader_parameter("grain_scale", 1.7)
	material.set_shader_parameter("wet_width", 14.0)
	material.set_shader_parameter("sheet_strength", 0.9)
	...
	# Sparse shell and pebble scatter along the tide line.
	...
	for i in count:
		...
		var shade := rng.randf_range(0.65, 0.9)
		mm.set_instance_color(i, Color(shade, shade * 0.92, shade * 0.85) if rng.randf() < 0.75 else Color(0.4, 0.36, 0.3))
