...
# Golden sand beach: smooth packed grain, wide wet band with a strong mirror
# sheet, shells and pebbles scattered along the tide line.
...
func build() -> void:
	...
	material.set_shader_parameter("grain_scale", 0.85)
	material.set_shader_parameter("wet_width", 14.0)
	material.set_shader_parameter("sheet_strength", 1.6)
	material.set_shader_parameter("ripple_depth", 0.25)
	material.set_shader_parameter("mottle", 0.6)
	...
	for i in count:
		...
		mm.set_instance_color(i, Color(shade, shade * 0.9, shade * 0.8) if rng.randf() < 0.7 else Color(0.35, 0.33, 0.3))
