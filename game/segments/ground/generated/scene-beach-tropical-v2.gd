...
# Golden sand beach: fine grain, a wide wet band with a strong mirror sheet,
# and a sparse scattering of shells along the tide line.
...
func build() -> void:
	...
	material.set_shader_parameter("dry_color", Color(0.72, 0.55, 0.3))
	material.set_shader_parameter("wet_color", Color(0.42, 0.32, 0.16))
	material.set_shader_parameter("grain_scale", 1.7)
	material.set_shader_parameter("wet_width", 14.0)
	material.set_shader_parameter("sheet_strength", 1.6)
	material.set_shader_parameter("ripple_depth", 0.8)
	material.set_shader_parameter("mottle", 0.9)
	...
	# Few shells and pebbles scattered along the tide line, as one MultiMesh.
	...
	for i in count:
		var x := rng.randf_range(-8.0, 3.0) + (rng.randf_range(-30.0, 0.0) if rng.randf() < 0.2 else 0.0)
		...
		mm.set_instance_color(i, Color(shade, shade * 0.88, shade * 0.8) if rng.randf() < 0.7 else Color(0.36, 0.34, 0.3))
