...
func _hotels(parent: Node3D, rng: RandomNumberGenerator) -> void:
	...
	var bx := BeachContext.BOARDWALK_X
	while z < 0.0:
...
func _far_tower(parent: Node3D) -> void:
	...
	var batch := MeshBatch.new()
	var x := BeachContext.BOARDWALK_X - 120.0
...
func _lifeguard_tower(parent: Node3D, pos: Vector3, hut: Color, roof: Color) -> void:
	...
	var t := Node3D.new()
	t.position = Vector3(pos.x, ctx.ground_height(pos.x, pos.z), pos.z)
	parent.add_child(t)
