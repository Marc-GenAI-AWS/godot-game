...
# Beach furniture, normal density: dense-ish rows of white loungers with
# mixed bright fabric, some towels, rainbow-panelled umbrellas over about a
# third of the loungers, and scattered clutter (buckets, coolers, balls,
# bags). Exposes `spots` so the crowd layer can seat people.
...
func build() -> void:
	# Rainbow-leaning umbrella panel sets plus a couple of pastel-toned sets
	# so umbrellas read as rainbow without being too saturated.
	var sets := [
		[Color(0.95, 0.15, 0.15), Color(0.98, 0.6, 0.1), Color(0.98, 0.9, 0.15), Color(0.2, 0.75, 0.35), Color(0.2, 0.55, 0.95), Color(0.55, 0.25, 0.85)],
		[Color(0.95, 0.2, 0.55), Color(0.98, 0.8, 0.2), Color(0.25, 0.7, 0.95)],
		[Color(0.9, 0.3, 0.15), Color(0.98, 0.85, 0.3), Color(0.3, 0.6, 0.95), Color(0.2, 0.75, 0.4)],
		[Color(0.95, 0.5, 0.15), Color(0.98, 0.9, 0.5), Color(0.9, 0.3, 0.35)],
		[Color(0.2, 0.6, 0.9), Color(0.95, 0.95, 0.9), Color(0.9, 0.25, 0.25)],
	]
	for cols in sets:
...
func build_chunk(chunk: Node3D, rng: RandomNumberGenerator) -> void:
	...
	var chunk_spots := []
	var fabric := [Color(0.95, 0.25, 0.5), Color(0.98, 0.85, 0.2), Color(0.3, 0.55, 0.9), Color(0.3, 0.75, 0.45),
		Color(0.9, 0.35, 0.15), Color(0.95, 0.75, 0.15), Color(0.25, 0.7, 0.95)]
	...
	for row_x in [-13.0, -18.0, -23.0, -28.0, -34.0]:
		var occupancy := 0.75 if row_x < -20.0 else 0.55
...
func _clutter(flat: MeshBatch, panels: Array[MeshBatch], base: Transform3D, rng: RandomNumberGenerator) -> void:
	...
	if r < 0.3:
		var bc: Color = [Color(0.95, 0.2, 0.6), Color(0.2, 0.7, 0.9), Color(0.95, 0.85, 0.2)][rng.randi() % 3]
