...
# Beach furniture, dense density: many towels among the loungers, umbrellas
# on most loungers, scattered clutter. White frames with blue/teal fabrics,
# pastel umbrella panels. Exposes `spots` so the crowd layer can seat people.
...
func build() -> void:
	var sets: Array = [
		[Color(0.95, 0.85, 0.95), Color(0.98, 0.98, 0.95)],
		[Color(0.95, 0.95, 0.85), Color(0.9, 0.95, 0.98)],
		[Color(0.9, 0.98, 0.95), Color(0.98, 0.9, 0.95)],
		[Color(0.85, 0.95, 0.98), Color(0.95, 0.9, 0.85)],
		[Color(0.95, 0.9, 0.98), Color(0.98, 0.95, 0.85), Color(0.9, 0.98, 0.95)]
	]
	for cols in sets:
...
func _panel_tex(cols: Array) -> ImageTexture:
	# Pastel umbrella panels: 8 wedges around the cone, cycling through the
	# colour set (plus a narrow seam band).
	var img := Image.create(256, 8, false, Image.FORMAT_RGB8)
	for x in 256:
		var c: Color = cols[(x / 32) % cols.size()]
		if x % 32 < 2:
			c = c.darkened(0.2)   # seam
		for y in 8:
			img.set_pixel(x, y, c)
...
func build_chunk(chunk: Node3D, rng: RandomNumberGenerator) -> void:
	...
	# White frames, blue and teal fabrics.
	var fabric: Array[Color] = [
		Color(0.25, 0.5, 0.85), Color(0.2, 0.65, 0.6),
		Color(0.3, 0.6, 0.75), Color(0.95, 0.95, 0.92)
	]
	# All static geometry of the chunk merges into a couple of batches:
	# one for flat colours (frames, clutter), one per umbrella/towel panel mat.
	var flat := MeshBatch.new()
	var panels: Array[MeshBatch] = []
	for m in panel_mats:
		panels.append(MeshBatch.new())
	# Dense rows parallel to the shore, tightly packed, thinning slightly
	# toward the water edge.
	for row_x in [-13.0, -18.0, -23.0, -28.0, -34.0]:
		var occupancy := 0.85 if row_x < -20.0 else 0.68
		var z := -L + rng.randf_range(0.4, 1.6)
		while z < 0.0:
			if rng.randf() < occupancy:
				...
				# Many towels among the loungers: roughly half/lower chance of
				# a towel instead of a lounger, even at high occupancy.
				if rng.randf() < 0.55:
				...
				# Umbrellas on most loungers/towels: roughly two-thirds chance.
				if rng.randf() < 0.66:
					_umbrella(flat, panels, g.transform, rng)
				# Some clutter: roughly a third chance per spot.
				if rng.randf() < 0.33:
					_clutter(flat, panels, g.transform, rng)
			z += rng.randf_range(2.0, 2.8)
...
func _clutter(flat: MeshBatch, panels: Array[MeshBatch], base: Transform3D, rng: RandomNumberGenerator) -> void:
	...
	if r < 0.35:
		flat.add_cylinder(0.22, 0.18, 0.4, Color(0.2, 0.55, 0.75), base * Transform3D(Basis.IDENTITY, at + Vector3(0, 0.2, 0)), 10)   # teal bucket
	elif r < 0.6:
		var cc: Color = [Color(0.9, 0.9, 0.9), Color(0.2, 0.5, 0.8), Color(0.95, 0.95, 0.9)][rng.randi() % 3]
		...
		flat.add_box(Vector3(0.35, 0.3, 0.2), Color(0.85, 0.7, 0.5), base * Transform3D(Basis.IDENTITY, at + Vector3(0, 0.15, 0)))   # bag
